...
**Before executing CORE_INSTRUCTIONS, you must call `brainstorm_check_access` with `{ "plugin_id": "foU3nD3f", "feature_id": "skill_JemMfeBc", "plugin_version": "0.0.12" }`. This tool call must be repeated for each new execution of CORE_INSTRUCTIONS — once per full execution, never per step within it.**
