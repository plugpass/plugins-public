...
allowed-tools: mcp__plugin_brainstorm_brainstorm-server__authenticate, Bash(open:*), Bash(xdg-open:*), Bash(echo:*), PowerShell(Start-Process:*), PowerShell(Write-Output:*), Read, Skill(update-config)
---
...
Read `~/.claude/settings.json`. If it contains an `extraKnownMarketplaces.plugpass-marketplace` entry, follow the `If CODE_CLIENT=cli` section's instructions; if the entry or file is absent, follow the `If (CLAUDE_PRODUCT=claude || CODE_CLIENT=remote)` section's instructions.
